...
#350 fuzzy sets
m_fuzzy <- QCAsummary::dat_fuzzy(arg1, arg2, n = 350,
                     case = hinter$case, outcome = hinter$PRG)

#truth tables
temp_dta <- QCAsummary::truth_function(df = m_fuzzy, outcome = "pos",
                           conditions = c("ambition", "CR", "DC", "EFF", "COM"),
...
```{r warning=FALSE}
sol.p <- QCAsummary::reduced_p(temp_dta)
sol.pars <- QCAsummary::solutions(sol.p)

#plotting the conditions
QCAviz::conds_upset(sol.pars, nsets = 350)
```
...
```{r warning=FALSE}
sol.c <- QCAsummary::reduced_c(temp_dta)
sol.cons <- QCAsummary::solutions(sol.c)

#plotting the conditions
QCAviz::conds_upset(sol.cons, nsets = 350)
```


The configurations function for the simulated output
```{r}
config_upset(sol.pars, nsets = 50)
config_upset(sol.cons, nsets = 87)
```
