...
#' sols_upset()
#'
#' Function plots frequencies of multiple QCA solutions.
#'
#' @importFrom magrittr %>%
#' @import stringi
#' @param df Dataframe containing multiple QCA solutions.
#' @param nsets An argument imported from the \pkg{UpSetR}
#' package. Determines the number of sets to be plotted.
#' @return A bar chart with frequencies for multiple solutions.
#' @export
sols_upset <- function(df, nsets) {
  df <- unlist(df)
  temp <- purrr::map(df, function(x) stringi::stri_split_fixed(x, " "))
  all_values <- unlist(unique(Reduce(c, df)) %>% unlist())
  final_matrix <- plyr::ldply(df, function(y) comparison(x = all_values, y = y, num = T))
...
#UpSetR specified for the Panel data
...
#'
#' @export
conds_panel <- function(df, nsets, type) {
  temp1 <- df %>%
    filter(type == type)
  temp1 <- purrr::map(temp1$solution, function(x) stringi::stri_split_fixed(x, "*") %>% unlist())
  temp1 <- purrr::map(temp1, function(x) stringi::stri_split_fixed(x, "+") %>% unlist())
  all_values <- stringi::stri_unique(unlist(temp1))
  final_matrix <- plyr::ldply(temp1, function(y) comparison(x = all_values, y = y, num = T))
  colnames(final_matrix) <- all_values
...
#'
#' @export
config_panel <- function(df, nsets, type) {
temp1 <- df %>%
  filter(type == type)
temp1 <- purrr::map(temp1$solution, function(x) stringi::stri_split_fixed(x, "+"))
all_values <- stringi::stri_unique(unlist(temp1))
final_matrix <- plyr::ldply(temp1, function(y) comparison(x = all_values, y = y, num = T))
colnames(final_matrix) <- all_values
UpSetR::upset(final_matrix, order.by = "freq", nsets = nsets)
...
#' sols_panel()
#'
#' Plot Function for the charting of solutions
#' intersections of the panel QCA output.
#'
#' @importFrom magrittr %>%
#' @import stringi
#' @param df the data frame which has the
#' configurations extracted from the QCA output
#' in its entirety
#' @param nsets an argument imported from the UpSetR
#' package. Determines the number of sets to graph.
#' @param type an argument to specify the panel dimension.
...
#' visual represenation of the interconnecteds
#' between the conditions in a given dataset
#' @note this chart is different from that which
#' is generated by conds_upset
#'
#' @export
sols_panel <- function(df, nsets, type) {
  temp1 <- df %>%
    filter(type == type)

  df <- unlist(temp1$solution)
  temp <- purrr::map(temp1, function(x) stringi::stri_split_fixed(x, " "))
  all_values <- unlist(unique(Reduce(c, df)) %>% unlist())
  final_matrix <- plyr::ldply(df, function(y) comparison(x = all_values, y = y, num = T))
  colnames(final_matrix) <- paste0("Sol", 1:length(all_values))
  UpSetR::upset(final_matrix, order.by = "freq", nsets = nsets)
...
#'
#' @export
sols_upset_h <- function(df, nsets) {
  temp <- purrr::map(df, function(x) unlist (x))
  temp <- purrr::map(temp, function(x) paste(x, collapse = "+"))
  all_values <- unlist(unique(Reduce(c, temp)) %>% unlist())
  final_matrix <- plyr::ldply(temp, function(y) comparison(x = all_values, y = y, num = T))
  colnames(final_matrix) <- paste0("Sol", 1:length(all_values))
  UpSetR::upset(final_matrix, order.by = "freq", nsets = nsets)
}
